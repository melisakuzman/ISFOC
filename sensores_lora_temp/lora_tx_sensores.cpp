...
bool configurar_ds18b20()
{
  uint offset;

  if (!pio_can_add_program(pio, &onewire_program)) {
      puts("No se pudo agregar el programa PIO");
      return false;
  }

  offset = pio_add_program(pio, &onewire_program);

  if (!ow_init(&ow, pio, offset, pin_Temp)) {
      puts("No se pudo inicializar OW");
      return false;
  }

  uint64_t romcodes[1];
  int encontrados = ow_romsearch(&ow, romcodes, 1, OW_SEARCH_ROM);

  if (encontrados < 1) {
      puts("No se encontró ningún sensor");
      return false;
  }

  sensor_rom = romcodes[0];
...
int main()
{
  ...
  configurar_uart();
  configurar_radio();
  sensor_ds_OK = configurar_ds18b20();
  if (sensor_ds_OK){
    printf("Sensor temperatura configurado correctamente");
  }
    ...
    if (transmittedFlag)
    {
      ...
      for (int i = 0; i < 4; i++)
      {
        if (i == 3)
        {
          ...
          append_to_string(str, sizeof(str), "%.2f,",temp);
        }
